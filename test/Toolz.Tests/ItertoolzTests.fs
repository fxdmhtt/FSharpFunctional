...
let ``test_count`` () =
    ...
    count (seq { 1; 2; 3; 4 }) |> should equal 4
...
let ``test_interpose`` () =
    ...
    interpose 0 (Seq.replicate 4 1) |> toList |> should equal [1; 0; 1; 0; 1; 0; 1]
    interpose '.' ['a'; 'b'; 'c'] |> toList |> should equal ['a'; '.'; 'b'; '.'; 'c']
...
let ``test_isdistinct`` () =
    ...
    isdistinct (seq { 1; 2; 3 }) |> should equal true
    isdistinct (seq { 1; 2; 1 }) |> should equal false
...
[<Fact>]
let ``test_mapcat`` () =
    mapcat (fun s -> map Char.ToUpper s) [['a'; 'b']; ['c'; 'd'; 'e']] |> should equal ['A'; 'B'; 'C'; 'D'; 'E']

    mapcat identity [[1; 2; 3]; [4; 5; 6]] |> should equal [1; 2; 3; 4; 5; 6]

    mapcat rev [[3; 2; 1; 0]; [6; 5; 4]; [9; 8; 7]] |> should equal [0..9]

    let inc i = i + 1
    mapcat (map inc) [[3; 4; 5]; [6; 7; 8]] |> should equal [4; 5; 6; 7; 8; 9]

[<Fact>]
let ``test_nth`` () =
    nth 1 "ABC" |> should equal 'B'
    nth 2 "ABCDE" |> should equal 'C'
    nth 2 (seq { 'A'; 'B'; 'C'; 'D'; 'E' }) |> should equal 'C'
...
let ``test_partition`` () =
    ...
    partition 3 (seq { 0..6 }) |> toList |> should equal [[0; 1; 2]; [3; 4; 5]]
...
let ``test_topk`` () =
    ...
    topk 2 ["Alice"; "Bob"; "Charlie"; "Dan"] length |> should equal ["Charlie"; "Alice"]
...
let ``test_unique`` () =
    ...
    unique ["cat"; "mouse"; "dog"; "hen"] length |> toList |> should equal ["cat"; "mouse"]
